...
### Translations
- Think before adding multiple similar translation keys. Prefer reusable placeholder translations such as `Create :item` instead of separate keys like `Create Category`, `Create Post`, or `Create User`.
- For numbered or repeated labels, use placeholders such as `Category :i` instead of creating separate keys like `Category Level 1`, `Category Level 2`, and `Category Level 3`.
